...
6.  Colors are **0–1 range** (not 0–255): `{r: 1, g: 0, b: 0}` = red — see [gotchas-fills-and-nodes.md](references/gotchas-fills-and-nodes.md)
7.  Fills/strokes are **read-only arrays** — clone, modify, reassign — see [gotchas-fills-and-nodes.md](references/gotchas-fills-and-nodes.md)
...
12. **`layoutSizingHorizontal/Vertical = 'FILL'` MUST be set AFTER `parent.appendChild(child)`** — setting before append throws. Same applies to `'HUG'` on non-auto-layout nodes. — see [gotchas-auto-layout.md](references/gotchas-auto-layout.md)
13. **Position new top-level nodes away from (0,0).** Nodes appended directly to the page default to (0,0). Scan `figma.currentPage.children` to find a clear position (e.g., to the right of the rightmost node). This only applies to page-level nodes — nodes nested inside other frames or auto-layout containers are positioned by their parent. See [gotchas-fills-and-nodes.md](references/gotchas-fills-and-nodes.md).
14. **On `use_figma` error, STOP. Do NOT immediately retry.** Failed scripts are **atomic** — if a script errors, it is not executed at all and no changes are made to the file. Read the error message carefully, fix the script, then retry. See [Validation & Error Recovery](references/validation-and-recovery.md).
...
16. **Always set `variable.scopes` explicitly when creating variables.** The default `ALL_SCOPES` pollutes every property picker — almost never what you want. Use specific scopes like `["FRAME_FILL", "SHAPE_FILL"]` for backgrounds, `["TEXT_FILL"]` for text colors, `["GAP"]` for spacing, etc. See [variable-patterns.md](references/variable-patterns.md) for the full list — see [gotchas-variables.md](references/gotchas-variables.md)
...
> For detailed WRONG/CORRECT examples, load the relevant gotchas file:
> [gotchas-fills-and-nodes.md](references/gotchas-fills-and-nodes.md) · [gotchas-auto-layout.md](references/gotchas-auto-layout.md) · [gotchas-components.md](references/gotchas-components.md) · [gotchas-variables.md](references/gotchas-variables.md) · [gotchas-text.md](references/gotchas-text.md)
...
Load [validation-and-recovery.md](references/validation-and-recovery.md) when you need:
- Guidance on the incremental workflow (inspect → build → validate → fix)
- Help choosing between `get_metadata` and `get_screenshot`
- Error recovery steps after a failed `use_figma` call
- A table of common error messages and how to fix them

Load this file: at the start of any complex or multi-step task, when choosing between `get_metadata` and `get_screenshot`, or when recovering from a failed `use_figma` call.
...
## 7. Discover Conventions Before Creating
...
|-----|-------------|----------------|
| [gotchas-fills-and-nodes.md](references/gotchas-fills-and-nodes.md) | Creating shapes, frames, or applying fills/colors | Node positioning, color ranges, fill immutability, paint variable binding |
| [gotchas-auto-layout.md](references/gotchas-auto-layout.md) | Working with auto-layout, sizing modes, or nested frames | FILL/HUG ordering, resize() behavior, layoutGrow, grid layout, sections |
| [gotchas-components.md](references/gotchas-components.md) | Creating components, variants, or working with instances | combineAsVariants, component properties, variable modes per component, detachInstance |
| [gotchas-variables.md](references/gotchas-variables.md) | Creating variable collections, modes, or binding variables | Modes, scopes, COLOR rgba format, code syntax, TextStyle binding limits |
| [gotchas-text.md](references/gotchas-text.md) | Creating text nodes, loading fonts, or setting typography | lineHeight/letterSpacing format, font style name probing |
