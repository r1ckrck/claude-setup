...
function sameStemPythonGeneratorPath(stepPath) {
  ...
  // Generators live beside the artifact, or in a sibling src/ folder (project convention).
  const candidates = [
    path.join(dir, `${stem}.py`),
    path.join(dir, "src", `${stem}.py`),
  ];
  for (const candidate of candidates) {
    if (fileHasGenStep(candidate)) {
      return candidate;
    }
  }
  return "";
...
export function createLocalAssetBackend({
  ...
  function refreshCatalogForPythonSource({ rootDir: nextRootDir = defaultRootDir, filePath } = {}) {
    ...
    const generatorDir = path.dirname(resolvedFilePath);
    // A generator in a src/ folder maps to its .step in the parent dir (project convention).
    const stepDir = path.basename(generatorDir) === "src" ? path.dirname(generatorDir) : generatorDir;
    const sameStemStepPath = path.join(stepDir, `${path.basename(resolvedFilePath, ".py")}.step`);
