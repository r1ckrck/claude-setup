...
import { buildInspectorReadout } from "../../../workbench/inspectorReadout.js";
...
export function useViewerPicking({
  ...
  const onInspectReadoutChangeRef = useRef(onInspectReadoutChange);
  ...
  onInspectReadoutChangeRef.current = onInspectReadoutChange;
  ...
  useEffect(() => {
    ...
    if (!runtime || !mountRef.current || previewMode) {
      ...
      onInspectReadoutChangeRef.current?.(null);
      return;
    }

    // Tracks whether the live inspector readout has already been blanked, so leaving
    // the scene emits a single null rather than one per frame.
    let inspectorReadoutCleared = false;
    ...
    // The inspector reads face/edge topology independent of the activation pick mode, so it
    // works at the assembly whole-model level (where clicks target whole parts) as well as
    // inside an inspected part. Part mode resolves the picked face/edge up to its occurrence.
    function emitInspectorReadout(clientX, clientY) {
      const onChange = onInspectReadoutChangeRef.current;
      if (!onChange) {
        return;
      }
      setPointerFromPosition(clientX, clientY);
      const modelIntersections = intersectVisibleModelMeshes();
      const referenceId = pickTopologyReference(modelIntersections, clientX, clientY, { hover: true });
      const hit = modelIntersections[0];
      let hitPointModelXYZ = null;
      if (hit?.point && runtime.modelGroup) {
        const local = runtime.modelGroup.worldToLocal(hit.point.clone());
        hitPointModelXYZ = [local.x, local.y, local.z];
      }
      const selectorRuntime = selectorRuntimeRef.current;
      const reference = referenceId ? selectorRuntime?.referenceMap?.get?.(referenceId) || null : null;
      inspectorReadoutCleared = false;
      onChange(buildInspectorReadout({
        reference,
        hitPointModelXYZ,
        selectorRuntime,
        mode: selectModeRef.current
      }));
    }

    function clearInspectorReadout() {
      const onChange = onInspectReadoutChangeRef.current;
      if (!onChange || inspectorReadoutCleared) {
        return;
      }
      inspectorReadoutCleared = true;
      onChange(null);
    }
    ...
    function flushHoverPick() {
      ...
      const referenceId = pickReferenceAtPosition(hoverState.x, hoverState.y, { hover: true });
      commitHoverState(referenceId);
      emitInspectorReadout(hoverState.x, hoverState.y);
    }
