...
import CursorInspectorPanel from "./CursorInspectorPanel";
...
export default function CadRenderPane({
  ...
  const ctaLabel = "Copy Screenshot";
  const ctaTitle = "Copy screenshot to clipboard";
  const ctaDisabled = viewerLoading || !activeMeshData;
    ...
    <div className="absolute inset-0">
      {dxfMode && !dxfMeshPreviewReady ? (
        ...
        <CadViewer
          ...
          selectMode={dxfMode || pathPreviewMode ? undefined : selectMode}
          onHoverReferenceChange={handleModelHoverChange}
          onInspectReadoutChange={dxfMode || pathPreviewMode ? undefined : onInspectReadoutChange}
      ...
      {!previewMode && ctaMode === "screenshot" && !stepUpdateInProgress && !topologySelectionPending && !topologySelectionUnavailable && !topologySelectionDeferred ? (
        <div
      ...
      {!previewMode && inspectorActive ? (
        <CursorInspectorPanel
          readout={inspectReadout}
          selectMode={selectMode}
          viewportFrameInsets={viewportFrameInsets}
          onCopyStatus={onInspectCopyStatus}
          pinToken={inspectPinToken}
        />
      ) : null}
    </div>
  );
