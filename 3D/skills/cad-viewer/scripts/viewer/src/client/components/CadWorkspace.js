...
import { buildPartReadoutFromAssemblyPart } from "@/workbench/inspectorReadout";
import {
...
export default function CadWorkspace({
  ...
  const [inspectReadout, setInspectReadout] = useState(null);
  const [inspectPinToken, setInspectPinToken] = useState(0);
  const inspectReadoutRef = useRef(null);
  const inspectorPinEligibleRef = useRef(false);
  ...
  const handleModelReferenceActivate = useCallback((referenceId, { multiSelect = false } = {}) => {
    ...
    if (inspectorPinEligibleRef.current && inspectReadoutRef.current?.entity) {
      setInspectPinToken((token) => token + 1);
    }
    if (viewerInAssemblyMode) {
      const pickedPartId = String(referenceId || "").trim();
  ...
  inspectorPinEligibleRef.current = selectionToolActive;
  const inspectorInsidePart = Boolean(
    isAssemblyView
    && effectiveInspectedAssemblyNodeId
    && effectiveInspectedAssemblyNodeId !== assemblyRootNodeId
  );
  // Selection mode is fully context-driven: whole-model inspects parts, inside a part inspects features.
  const selectMode = inspectorInsidePart ? SELECT_MODE.FEATURE : SELECT_MODE.PART;
  // Topology loads only inside an inspected part; at the whole-model level Part mode reads the
  // hovered part's render data, keeping the live cursor XYZ the picking hook already computed.
  const wholeModelHoveredPart = useMemo(() => {
    if (!viewerInAssemblyMode || selectMode !== SELECT_MODE.PART) {
      return null;
    }
    const partId = String(hoveredModelPartId || "").trim();
    return partId ? assemblyPartMap.get(partId) || null : null;
  }, [viewerInAssemblyMode, selectMode, hoveredModelPartId, assemblyPartMap]);
  const effectiveInspectReadout = useMemo(() => {
    if (!selectionToolActive) {
      return null;
    }
    if (wholeModelHoveredPart) {
      return buildPartReadoutFromAssemblyPart({
        part: wholeModelHoveredPart,
        cadPath: cadPathForEntry(selectedEntry),
        cursorXyz: inspectReadout?.cursor?.xyz || null
      });
    }
    return inspectReadout;
  }, [selectionToolActive, wholeModelHoveredPart, selectedEntry, inspectReadout]);
  inspectReadoutRef.current = effectiveInspectReadout;
    ...
    >
      <div className="fixed inset-0 z-0">
        <CadRenderPane
          ...
          onInspectReadoutChange={selectionToolActive ? setInspectReadout : undefined}
          inspectReadout={effectiveInspectReadout}
          inspectorActive={selectionToolActive}
          inspectPinToken={inspectPinToken}
          onInspectCopyStatus={setCopyStatus}
