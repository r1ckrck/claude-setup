...
function pythonIdentityRootCandidates({ repoRoot, sourceFilePath, anchorPath = "", preferredRoot = null }) {
  ...
  // The generator's own directory is a valid identity root and is what cadpy uses for the
  // `models/src/` convention (generator in src/, artifacts one level up). The commonRoot
  // walk below starts at the generator/STEP common ancestor (the models dir) and only
  // climbs, so src/ is never tried without this seed — producing a permanent false
  // "stale_source_identity" (and losing selector topology) for every src/-layout model.
  roots.push(path.dirname(path.resolve(sourceFilePath)));
  if (commonRoot) {
