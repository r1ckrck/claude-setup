...
// ---- Clip caps: fill the cut face when a section plane slices a solid (stencil technique) ----
// Per part i: stencil masks at B+2i, the part's colored cap quad at B+2i+1. The cap resets the
// stencil to 0 as it draws, so the next part starts clean — per-part colors with no manual clears.
// Base 10 sits above the edge lines (renderOrder 3) so every cap sorts consistently over them.
const CLIP_CAP_BASE_RENDER_ORDER = 10;
const CLIP_CAP_OVERSCAN = 1.2;
const CLIP_CAP_FALLBACK_COLOR = "#9ca3af";
...
// Pure: darken the theme surface color for the cap; neutral fallback when unparseable.
export function resolveClipCapColor(runtime, factor = 0.85) {
  const surface = typeof runtime?.baseTheme?.surface === "string" ? runtime.baseTheme.surface.trim() : "";
  const match = /^#([0-9a-fA-F]{6})$/.exec(surface);
  if (!match) {
    return CLIP_CAP_FALLBACK_COLOR;
  }
  const hex = match[1];
  const channels = [0, 2, 4].map((start) => clampColorByte(parseInt(hex.slice(start, start + 2), 16) * factor));
  return `#${channels.map((c) => c.toString(16).padStart(2, "0")).join("")}`;
}

// Pure: a part's cap takes that part's own body color, lightly darkened. Reads record.baseColor
// (the stable color; NOT material.color, which hover/selection temporarily overwrites), falling back
// to material.color, then the theme neutral. Vertex-color parts have a white baseColor (misleading),
// so they use the theme neutral instead.
export function clipCapColorForRecord(runtime, record, factor = 0.82) {
  if (record?.useVertexColors === true) {
    return resolveClipCapColor(runtime, factor);
  }
  const color = record?.baseColor || record?.material?.color || null;
...
// Pure gate: caps only in solid display mode, with a model, a plane, and clipping enabled.
export function shouldRenderClipCaps({ enabled, displayMode, hasRecords, hasPlane } = {}) {
  if (!enabled || !hasPlane || !hasRecords) {
    return false;
  }
  return displayMode !== CAD_DISPLAY_MODE.WIREFRAME;
}

// Two stencil-only mask materials per part: back faces increment, front faces decrement (wrap).
// Matches three.js webgl_clipping_stencil. Clipped by the active plane; never writes color/depth.
export function buildClipCapStencilMaterials(THREE, plane) {
  const base = {
    depthWrite: false,
    depthTest: false,
    colorWrite: false,
    transparent: true, // keep masks in the same (transparent) render queue as the caps so per-part interleaving holds
    clipping: true,
...
// The cap quad fills wherever the stencil is non-zero (inside solid) and resets it to 0.
// The cap itself must NOT be clipped (it defines the cut face). Always `transparent` (so all caps +
// masks share the transparent queue and interleave correctly); `opacity` tracks the part's display
// opacity so dimmed/focused parts' caps dim with their bodies. Opaque-looking caps write depth.
export function buildClipCapPlaneMaterial(THREE, color, opacity = 1) {
  return new THREE.MeshBasicMaterial({
    color,
    side: THREE.DoubleSide,
    clipping: false,
...
function clipCapRecordOpacity(record) {
  const raw = record?.material?.opacity;
  return Number.isFinite(Number(raw)) ? Math.max(0, Math.min(1, Number(raw))) : 1;
}

// Update existing cap + mask opacity/visibility in place from each part's current display state
// (focus dimming, hide/show). Cheaper than a rebuild — called after applyPartVisualState. The cap's
// `transparent` flag stays fixed (queue placement); only opacity/depthWrite/visibility change.
export function syncClipCapStyle(runtime) {
  const capsGroup = runtime?.capsGroup;
  if (!capsGroup || capsGroup.visible === false) {
    return;
  }
  const masks = Array.isArray(runtime?.clipCapStencilMeshes) ? runtime.clipCapStencilMeshes : [];
  for (const mask of masks) {
    const record = mask?.userData?.cadClipCapRecord;
    mask.visible = !record || record.mesh?.visible !== false;
  }
  for (const cap of capsGroup.children) {
    const record = cap?.userData?.cadClipCapRecord;
    if (!record || !cap.material) {
      continue;
    }
    cap.visible = record.mesh?.visible !== false;
    const opacity = clipCapRecordOpacity(record);
    cap.material.opacity = opacity;
    cap.material.depthWrite = opacity >= 0.999;
  }
}

export function disposeClipCapStencilMeshes(runtime) {
  const meshes = Array.isArray(runtime?.clipCapStencilMeshes) ? runtime.clipCapStencilMeshes : [];
  for (const mesh of meshes) {
    mesh?.parent?.remove(mesh);
    disposeMaterial(mesh?.material); // geometry is the shared/cached part geometry — do not dispose it
  }
  if (runtime) {
    runtime.clipCapStencilMeshes = [];
  }
}

// Build/update the per-part section caps for the current clip plane, or tear them down. Called at the
// end of syncRuntimeStepClipPlane, so runtime.activeClipPlane / displayRecords are current. Each part
// gets its own stencil masks (renderOrder B+2i) then its own colored cap (B+2i+1); the cap resets the
// stencil to 0 as it draws, so the next part reads only its own region — per-part colors, no clears.
export function syncRuntimeClipCaps(runtime, clipSettings) {
  const THREE = runtime?.THREE;
  const capsGroup = runtime?.capsGroup;
  if (!THREE || !capsGroup) {
    return;
  }
  const plane = runtime.activeClipPlane || null;
  ...
  const render = shouldRenderClipCaps({
    enabled: normalized.enabled,
    displayMode: runtime.displayMode,
    hasRecords: records.length > 0,
    hasPlane: Boolean(plane)
  });
  disposeClipCapStencilMeshes(runtime);
  clearGroup(capsGroup);
  if (!render) {
    capsGroup.visible = false;
    return;
  }
  const radius = Number.isFinite(Number(runtime.modelRadius)) ? Number(runtime.modelRadius) : 1;
  const size = Math.max(radius, 1e-3) * 2 * CLIP_CAP_OVERSCAN;
  ...
  for (const record of records) {
    ...
    const geometry = record?.geometry || sourceMesh?.geometry;
    if (!sourceMesh || !geometry || sourceMesh.visible === false) {
      continue; // identical filter for masks + cap so the index i pairs them
    }
    ...
    for (const material of [back, front]) {
      ...
      mask.renderOrder = CLIP_CAP_BASE_RENDER_ORDER + i * 2;
      mask.raycast = noopRaycast; // never a picking target
      mask.userData = { cadClipCapStencil: true, cadClipCapRecord: record };
      (sourceMesh.parent || runtime.modelGroup)?.add(mask);
      masks.push(mask);
    }
    const capMesh = new THREE.Mesh(
      new THREE.PlaneGeometry(size, size),
      buildClipCapPlaneMaterial(THREE, clipCapColorForRecord(runtime, record), clipCapRecordOpacity(record))
    );
    capMesh.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), normal);
    capMesh.position.copy(coplanar);
    capMesh.renderOrder = CLIP_CAP_BASE_RENDER_ORDER + i * 2 + 1;
    capMesh.frustumCulled = false;
    capMesh.raycast = noopRaycast;
    capMesh.userData = { cadClipCapRecord: record };
    capsGroup.add(capMesh);
    i += 1;
  }
  runtime.clipCapStencilMeshes = masks;
  capsGroup.visible = i > 0;
}
