...
from cadpy.stl import export_part_stl_from_scene, export_per_part_stls
...
from cadpy.threemf import export_part_3mf_from_scene, export_flat_3mf_from_scene
...
class EntrySpec:
    ...
    per_part_stl_dir: Path | None = None
...
def _resolve_step_option_output_dir(raw_output: str, *, base_step_path: Path) -> Path:
    value = str(raw_output or "").strip()
    if not value:
        raise ValueError("meshes-dir must be a non-empty path")
    if "\\" in value:
        raise ValueError("meshes-dir must use POSIX '/' separators")
    pure = PurePosixPath(value)
    if pure.is_absolute() or any(part in {"", "."} for part in pure.parts):
        raise ValueError("meshes-dir must be relative")
    return (base_step_path.resolve().parent / Path(*pure.parts)).resolve()
...
def _apply_step_options_to_spec(spec: EntrySpec, step_options: StepImportOptions) -> EntrySpec:
    ...
    # Combined STL — on by default; --no-stl disables; --stl PATH overrides the location.
    if step_options.no_stl:
        stl_path = None
    elif step_options.stl is not None:
        ...
        stl_path = base.resolve().with_suffix(".stl")

    # Flat multi-part 3MF — on by default; --no-3mf disables; --3mf PATH overrides the location.
    if step_options.no_3mf:
        three_mf_path = None
    elif step_options.three_mf is not None:
        ...
        three_mf_path = base.resolve().with_suffix(".3mf")

    # Per-part STLs — on by default; --no-meshes disables; --meshes-dir DIR overrides the location.
    if step_options.no_meshes:
        per_part_stl_dir = None
    elif step_options.meshes_dir is not None:
        per_part_stl_dir = _resolve_step_option_output_dir(step_options.meshes_dir, base_step_path=base)
    else:
        per_part_stl_dir = base.resolve().parent / "meshes"

    # Native GLB — opt-in only.
    ...
    return replace(
        ...
        per_part_stl_dir=per_part_stl_dir,
...
def _generate_part_outputs(
    ...
    *,
    ...
    if spec.per_part_stl_dir is not None:
        def per_part_stl_job() -> Path:
            paths = export_per_part_stls(scene, spec.per_part_stl_dir)
            return paths[0] if paths else spec.per_part_stl_dir

        jobs.append(_ArtifactJob("per-part STL", per_part_stl_job))

    if spec.three_mf_path is not None:
        def three_mf_sidecar_job() -> Path:
            return export_flat_3mf_from_scene(scene, spec.three_mf_path, color=spec.color)
