...
When a joint expresses real relative **motion** (not just a static pose), the geometry must physically realize that degree of freedom — a journal, a member, and retention. After positioning, run the moving-interface gate in `references/mechanism-verification.md`.
