...
9. **Verify declared motion is realized.** If the assembly has parts that move relative to each other, apply the moving-interface gate in `references/mechanism-verification.md`: every moving interface must have its realizing feature (journal + member + retention) present in the STEP, confirmed via `inspect`/`$cad-slice`. No-op for single parts and all-fixed/fastened assemblies.
10. **Verify primary STEP visually with snapshots.** After creating or visibly updating a primary STEP/STP part or assembly, ALWAYS run CAD `scripts/snapshot` against the primary STEP/STP artifact for visual verification/review. Do not skip snapshots for speed, convenience, confidence, or because deterministic checks passed. Skip only when `references/snapshot-review.md` says no visible geometry changed or no valid artifact exists, and report that reason. Use PNGs for static reviews and GIFs for motion/animation reviews; `scripts/inspect`, measurements, mating checks, frames, and diffs are complementary, not replacements.
...
- `references/mechanism-verification.md` — assemblies with parts that move relative to each other: the interface table and the journal/member/retention gate that confirms each declared degree of freedom is physically realized.
